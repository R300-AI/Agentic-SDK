...
BCI 射箭教練要解決的，不只是多模態訊號能不能被看見，而是教練能不能在同一回合內把這些訊號轉成可採取的調整方向。這個案例真正建立的，是一條會隨提問持續推進、會隨證據持續修正、並能保留上一輪判讀脈絡的訓練決策主線。
...
以「後三箭為何失穩」為例，workflow 的第一輪任務，是先把分散訊號收斂成同一段可推進的判讀流程。系統接到問題之後，會先從當輪輸入整理出真正需要優先處理的條件，再沿著這組條件回查對應的歷史片段與標註內容。最後得到的，不會只是零散觀察，而是一段把當輪異常、過往證據與修正方向整合過的建議，讓教練能立即進入調整。

這條主線之所以能成立，是因為 Agentic SDK 把每個模組固定在清楚的責任位置上。Perceive 負責整理多模態輸入，將腦波、姿態、動作抖動、心率與主觀回饋收斂成可判讀條件；Plan 依照這些條件決定本輪需要優先追查哪些歷史片段與標註脈絡；Retrieve 把對應證據帶回來；Action 將條件與證據組合成教練可以直接採取的建議；Reflect 再把整輪回應收斂成一致格式，確保下一輪還能沿用同一條分析主線。這一層讓讀者真正看到的，不只是 workflow 存在，而是 Agentic SDK 如何把不同職責穩定接起來。
