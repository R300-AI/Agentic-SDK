...
LaNew 售鞋顧問要解決的，不只是足測資料能不能被看懂，而是門市能不能在同一段對話裡，把足測結果轉成顧客願意採納的選鞋判斷。這個案例真正建立的，是一條會隨追問持續推進、會隨商品證據持續修正、並能保留前一輪推薦脈絡的導購決策主線。
...
以「我每天久站通勤，該選哪款」為例，workflow 的第一輪任務，是先把足測報告與當輪需求收斂成同一段可推進的推薦流程。系統接到問題之後，會先從足弓狀態、受力分布、尺寸風險與使用情境整理出真正需要優先處理的條件，再沿著這組條件回查候選鞋款的支撐特性、楦頭設定與材質差異。最後得到的，不會只是商品資料的羅列，而是一段把足測欄位、鞋款證據與穿著需求整合過的推薦說明，讓顧客第一次就能理解這雙鞋為什麼被提出。

這條主線之所以能成立，是因為 Agentic SDK 把每個模組固定在清楚的責任位置上。Perceive 負責整理足測欄位與提問，將足弓、受力、尺寸風險與使用需求收斂成可判讀條件；Plan 依照這些條件決定本輪需要優先比對哪些商品，或還需要補問哪些資訊；Retrieve 把尺寸、支撐與材質證據帶回來；Action 將條件與商品差異組合成顧客可以直接理解的推薦理由；Reflect 再把整輪回應收斂成一致格式，確保下一輪還能沿用同一條推薦主線。這一層讓讀者真正看到的，不只是 workflow 有在運作，而是 Agentic SDK 如何把不同職責穩定接起來。
