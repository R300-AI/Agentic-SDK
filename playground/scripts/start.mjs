...
async function startVite() {
  ...
  vite.on("error", (e) => process.stderr.write(`[playground] spawn error: ${e.message}\n`));
  vite.on("exit", (code) => { gw.kill(); process.exit(code ?? 0); });

  process.stdout.write(
    `${green}[playground]${reset} Gateway: http://localhost:8080 / UI: http://localhost:5173\n`
  );
}

startVite().catch((e) => {
  process.stderr.write(`[playground] startup error: ${e.message}\n`);
  gw.kill();
  process.exit(1);
});
