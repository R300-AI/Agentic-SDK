...
export function configToFlow(config: WorkflowConfig): { nodes: Node[]; edges: Edge[] } {
  ...
  const nodes: Node[] = NODE_DISPLAY_ORDER.map((name) => {
    const fallback: NodeSpec = {
      type: defaultTypeFor(name),
      params: defaultParamsFor(name),
      ...(name === "action" ? { compute_target: "cloud" } : {}),
    };
    const spec = config.nodes[name] ?? fallback;
    return {
      id: name,
      type: name,
...
function defaultParamsFor(name: string): Record<string, unknown> | undefined {
  if (name === "perceive") return {
  ...
  if (name === "plan") return {
    system_prompt:
      ...
      "規則（必須按優先順序判斷）：\n" +
      "1. 若 has_retrieved_context=true：必選 action，避免重複檢索。\n" +
