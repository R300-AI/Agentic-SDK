...
class MissingEndpointCredentials(ValueError):
    def __init__(self, role_label: str, endpoint: ModelEndpoint | None, missing_envs: list[str]) -> None:
        ...
        if endpoint is None:
            super().__init__(f"{role_label} has no model endpoint configured. Add {missing} in .env.")
        else:
            super().__init__(f"{role_label} uses {endpoint.label}, but .env is missing {missing}.")
...
def _openai_requirements(config: BuilderSourceConfig) -> list[OpenAIRequirement]:
    ...
    if "reflect" in reachable_roles:
        requirements.append(OpenAIRequirement("reflect", "回覆檢查器", "ResponseCheckReflect", "Reflect"))
    return requirements
