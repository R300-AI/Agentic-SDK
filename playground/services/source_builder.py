...
_STRUCTURED_RESULT_PROMPT = "請輸出有效 JSON；不得加入 Markdown code block 或額外解說。"
_OUTPUT_FORMAT_PROMPTS = {
    "free_text": "請依使用者需求自然回覆；語氣、角色、品牌口吻與呈現方式以使用者設定的回覆風格與規範為準。",
    "interactive": "請同時支援純文字回覆與可互動元件。一般問題可自然回答；當需要使用者選擇或填寫資料時，請呼叫工具，讓 Runner 依欄位收集資料後提交 API。",
}
...
def get_builder_steps() -> list[BuilderStep]:
    return [
        BuilderStep(
            ...
            "",
            (
                BuilderChoice("Minimal", "直接回答", "不預設外部 OpenAI 相容 client，適合先從最小流程開始。"),
...
def build_python_source_from_builder_choice(step_key: str, choice_label: object, existing_source: str | None) -> str:
    ...
    if step_key == "action":
        ...
        action_module = {
            ...
            "Custom": "CustomAction",
            "free_text": "GenerativeAction",
            "interactive": "ToolCallAction",
        }.get(str(choice_label), config.action_module)
        if str(choice_label) in {"ToolCall", "interactive"}:
            return _build_source_for_config(
                _replace_config(
                    config,
                    action_module="ToolCallAction",
                    profile_hint="Tool Call",
                    workflow_name=_workflow_name_for_profile(existing_source, "Tool Call Helper"),
                    action_prompt=config.action_prompt or _OUTPUT_FORMAT_PROMPTS["interactive"],
                    action_tools=config.action_tools or _tools_from_action_payload(
                        {
                            "interaction_trigger": "需要使用者確認或補充資料時呼叫。",
        ...
        if str(choice_label) == "free_text":
            profile_hint = "OpenAI Client"
            workflow_name = _workflow_name_for_profile(existing_source, "OpenAI Client Helper")
        ...
        action_prompt = config.action_prompt if action_module == "GenerativeAction" else None
        if str(choice_label) == "free_text" and not action_prompt:
            action_prompt = _OUTPUT_FORMAT_PROMPTS["free_text"]
        if structured_choice and not action_prompt:
            action_prompt = _STRUCTURED_RESULT_PROMPT
        return _build_source_for_config(
            _replace_config(
                config,
                action_module=action_module,
...
def _config_from_source(existing_source: str | None) -> BuilderSourceConfig:
    ...
    action_prompt = _extract_keyword_value(source, {"GenerativeAction", "ToolCallAction"}, "system_prompt") or _clean_prompt(str(action_config.get("output_guidance", ""))) or None
...
def get_builder_form_state(python_source: str, *, include_generated_defaults: bool = False) -> dict[str, object]:
    ...
    _add_form_value(values, "action", "response_instruction", _configured_text(config.action_prompt, _OUTPUT_FORMAT_PROMPTS.get("free_text"), include_generated_defaults))
    if config.action_module == "ToolCallAction":
        _add_form_value(values, "action", "component_fields", _pairs_text_from_action_tools(config.action_tools))
...
def _builder_choices_for_config(config: BuilderSourceConfig) -> dict[str, str]:
    ...
    action_choice = "interactive" if config.action_module == "ToolCallAction" else "free_text"
    reflect_choice = {
    ...
    return {
        "task_type": template_choice,
        "input_type": config.input_kind,
        "retrieve_policy": retrieve_choice,
        "output_format": action_choice,
        "failure_policy": reflect_choice,
    }
...
def _pairs_text_from_action_tools(tools: tuple[dict[str, object], ...]) -> str:
    for tool in tools:
        ...
        for name, schema in properties.items():
            ...
            type_hint = {"number": "數字", "boolean": "是否"}.get(field_type, "")
            label = f"{type_hint}：{description}" if type_hint and type_hint not in description else description
...
def _action_prompt_from_payload(payload: dict[str, Any], current_prompt: str | None) -> str | None:
    keys = set(payload)
    interactive_keys = {"interaction_trigger", "api_method", "api_url", "component_fields", "api_body_template", "missing_info_rule"}
    ...
    if {"response_instruction", *interactive_keys} & keys and (_prompt_looks_interactive(current_prompt) or interactive_keys & keys):
...
def _prompt_looks_interactive(current_prompt: str | None) -> bool:
    return bool(current_prompt and ("可互動元件" in current_prompt or "component.fields" in current_prompt or "api.url" in current_prompt or "呼叫工具" in current_prompt))
...
def _interactive_action_prompt(payload: dict[str, Any], current_prompt: str | None) -> str:
    ...
    api_body_template = _clean_prompt(str(payload.get("api_body_template", "")))
    missing_info_rule = _clean_prompt(str(payload.get("missing_info_rule", "")))
    parts = [current_prompt if _prompt_looks_interactive(current_prompt) else _OUTPUT_FORMAT_PROMPTS["interactive"]]
    if response_instruction:
        parts.append(f"回覆風格與規範：{response_instruction}")
    if trigger:
        parts.append(f"互動元件功能說明：{trigger}")
    parts.append(f"API 提交設定：{api_method}{(' ' + api_url) if api_url else ''}")
    if fields:
        parts.append(f"JSON Spec 需要的抽取欄位：\n{fields}")
    if api_body_template:
        parts.append(f"送出 JSON Body 範本：{api_body_template}")
    if missing_info_rule:
        parts.append(f"缺資料時的處理：{missing_info_rule}")
    return "\n".join(parts)


def _payload_has_interactive_contract(payload: dict[str, Any]) -> bool:
    return bool({"interaction_trigger", "api_method", "api_url", "component_fields", "api_body_template", "missing_info_rule", "api_contracts"} & set(payload))


def _tools_from_action_payload(payload: dict[str, Any]) -> tuple[dict[str, object], ...]:
    fields = _tool_fields_from_text(str(payload.get("component_fields", "")))
    if not fields:
        fields = ({"name": "confirm", "label": "確認送出", "type": "boolean"},)
    properties = {
        str(field["name"]): {
            "type": str(field["type"]),
            "description": str(field["label"]),
        }
        for field in fields
    }
    required = [str(field["name"]) for field in fields]
    trigger = _clean_prompt(str(payload.get("interaction_trigger", ""))) or "需要使用者確認或補充資料時呼叫。"
    method = _clean_short_text(str(payload.get("api_method", "POST")), "POST").upper()
    api_url = _clean_short_text(str(payload.get("api_url", "")), "")
    api_body_template = _clean_prompt(str(payload.get("api_body_template", "")))
    missing_info_rule = _clean_prompt(str(payload.get("missing_info_rule", "")))
    api_hint_parts = [f"API: {method} {api_url}" if api_url else f"API method: {method}"]
    if api_body_template:
        api_hint_parts.append(f"body: {api_body_template}")
    if missing_info_rule:
        api_hint_parts.append(f"missing info: {missing_info_rule}")
    api_hint = " | " + " | ".join(api_hint_parts)
    return (
        {
            "type": "function",
            "function": {
                "name": "submit_api_1",
                "description": f"{trigger}{api_hint}",
                "parameters": {
                    "type": "object",
...
def _tool_fields_from_text(text: str) -> tuple[dict[str, str], ...]:
    ...
    for line in str(text or "").splitlines():
        ...
        if parsed is None:
            continue
        name, label = parsed
        field_name = _clean_identifier_text(name, "field")
        field_label = _clean_short_text(label, field_name)
        fields.append({"name": field_name, "label": field_label, "type": _tool_field_type(label)})
    return tuple(fields)


def _tool_field_type(label: str) -> str:
    normalized = str(label or "").lower()
    if "boolean" in normalized or "true/false" in normalized or "是/否" in normalized or "確認" in normalized:
        return "boolean"
    if "number" in normalized or "integer" in normalized or "float" in normalized or "數字" in normalized or "金額" in normalized:
        return "number"
    return "string"
...
def _normalize_tool_items(value: object) -> list[dict[str, object]]:
    if not isinstance(value, list):
        return []
...
def _build_workflow_source(config: BuilderSourceConfig) -> str:
    ...
    hint = f"# Playground profile hint: {config.profile_hint}\n" if config.profile_hint else ""
...
# Playground profile hint: Custom Action
workflow = Workflow(
    workflow_name={workflow_name_literal},
    gates=Gates(max_node_hops={config.max_node_hops}, max_revisit={config.max_revisit}, timeout_sec={config.timeout_sec}),
    entry_module={json.dumps(config.entry_module, ensure_ascii=False)},
...
def _needs_openai_client(config: BuilderSourceConfig) -> bool:
    return bool(
        ...
        or config.action_module in {"GenerativeAction", "ToolCallAction"}
...
def _action_expression(config: BuilderSourceConfig) -> str:
    ...
    if action_class == "ToolCallAction" and config.action_tools:
        arguments.append(f"tools={_format_python_literal(list(config.action_tools), 4)}")
...
def _llm_env_prefixes(config: BuilderSourceConfig) -> list[str]:
    ...
    if config.action_module in {"GenerativeAction", "ToolCallAction"}:
