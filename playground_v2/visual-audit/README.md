...
```powershell
python.exe -m flask --app playground_v2.app:app run --host 127.0.0.1 --port 5051
```
...
node playground_v2/visual-audit/capture-r7.cjs
```
...
- `blueprint/09_visual_north_star_and_references.md`
- `blueprint/11_visual_design_tokens_motion.md`
- `blueprint/29_visual_art_consistency_spec.md`
...
$playwrightPackage = Get-ChildItem "$env:LOCALAPPDATA\npm-cache\_npx" -Recurse -Filter package.json | Where-Object { Select-String -Path $_.FullName -Pattern '"name": "playwright"' -Quiet } | Select-Object -First 1
$env:NODE_PATH = Split-Path $playwrightPackage.FullName -Parent | Split-Path -Parent
node playground_v2/visual-audit/keyboard-audit.cjs
```
