...
def test_runner_edit_settings_rehydrates_configured_builder_state():
    ...
    assert state["choices"]["task_type"] == "Custom Action"
    assert state["choices"]["output_format"] == "free_text"
...
def test_builder_key_value_parameters_use_repeatable_pair_editors():
    ...
    expected_outputs = (
        ...
        ('id="template-action-custom-rule-pairs" name="rule_pairs" hidden data-pair-output', "新增處理規則"),
        ('id="v3-action-component-fields" name="component_fields" hidden data-pair-output', "新增欄位"),
    )
...
def test_builder_finite_parameters_use_constrained_controls():
    ...
    assert response.status_code == 200
    page = response.data.decode("utf-8")
    assert '<select id="template-action-direct-memory-key" name="direct_memory_key">' in page
    assert '<select id="v3-action-api-method" name="api_method">' in page
    assert '<input id="template-action-direct-memory-key"' not in page
...
def test_builder_expanded_module_choices_update_canonical_source():
    ...
    assert "GenerativeAction(api_key=ACTION_API_KEY, base_url=ACTION_API_BASE_URL, model=ACTION_MODEL" in structured_source
    assert "請輸出有效 JSON" in structured_source
...
def test_builder_readme_starter_templates_update_source():
    ...
    assert "可直接發送的正式回覆" in openai_source
...
def test_builder_workflow_templates_map_to_sdk_module_presets():
    ...
    assert "請用一步步詢問" in advisor_source
...
def test_builder_advanced_action_choice_updates_source():
    ...
    assert "GenerativeAction(api_key=ACTION_API_KEY, base_url=ACTION_API_BASE_URL, model=ACTION_MODEL" in preview_response.data.decode("utf-8")
    assert "請輸出有效 JSON" in preview_response.data.decode("utf-8")
...
def test_builder_action_choice_clears_stale_action_specific_state():
    ...
    removed_action_name = "Structured" + "Action"
    assert removed_action_name not in reply_preview
    assert "GenerativeAction" not in reply_preview
    assert "system_prompt" not in reply_preview
    assert "Playground profile hint: Structured Result" not in reply_preview
...
def test_runner_uses_source_derived_layout_surfaces():
    ...
    assert "結構化需求內容".encode("utf-8") not in form_response.data
    assert b"structured-input-grid" not in form_response.data
