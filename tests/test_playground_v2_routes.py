import json
import re
from pathlib import Path

from playground_v2 import create_app


def _visible_text(html: str) -> str:
    return re.sub(r"<[^>]+>", " ", html)


def _builder_form_state(html: str) -> dict[str, object]:
    match = re.search(r'<script id="builder-form-state" type="application/json" data-builder-form-state>(.*?)</script>', html, re.S)
    assert match is not None
    return json.loads(match.group(1))


def test_entry_route_renders():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground")

    assert response.status_code == 200
    assert "AI Hub Agent 建立工具".encode("utf-8") in response.data
    assert "使用 AI Hub 登入".encode("utf-8") in response.data
    assert "不登入，先在本機試用".encode("utf-8") in response.data
    assert b"data-login-status" in response.data


def test_builder_route_renders_oobe_shell():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    assert "這個 Agent 要如何命名？".encode("utf-8") in response.data
    assert b"data-step-panel=\"reflect\"" in response.data
    assert "前往試跑頁".encode("utf-8") in response.data
    assert "輸出固定格式".encode("utf-8") in response.data
    assert "Agent 設定".encode("utf-8") in response.data
    assert "本機設定".encode("utf-8") in response.data
    assert "可試跑".encode("utf-8") in response.data
    assert b"draft-panel" not in response.data
    assert b"summary-list" not in response.data
    assert "目前階段：步驟 1 / 9".encode("utf-8") in response.data
    assert "aria-label=\"目前階段：步驟 1 / 9：這個 Agent 要如何命名？\"".encode("utf-8") in response.data
    assert b"progress-step-list" in response.data
    page = response.data.decode("utf-8")
    rail_start = page.index("progress-step-list")
    rail_end = page.index("</nav>", rail_start)
    rail_markup = page[rail_start:rail_end]
    assert "<button" not in rail_markup
    assert "aria-selected" not in rail_markup
    assert "<strong>" not in rail_markup
    assert "<small>" not in rail_markup
    for forbidden_copy in ("目前草稿", "設定摘要", "讓使用者一進入頁面", "目前 PoC", "README", "Gateway", "前端骨架"):
        assert forbidden_copy not in page
    assert b"WorkflowSettings" not in response.data


def test_builder_visible_language_hides_internal_module_terms():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    visible_text = _visible_text(response.data.decode("utf-8"))
    for business_copy in (
        "整理成自然回覆",
        "需求判斷",
        "套用作業規則",
        "依意思找相關資料",
        "規則處理進階配置",
        "規則 key 對處理 value",
        "從哪一步開始",
    ):
        assert business_copy in visible_text
    assert "回覆服務名稱" not in visible_text
    assert "回覆部署名稱" not in visible_text
    for internal_term in ("店內", "模型", "模組", "Action", "Perceive", "Retrieve", "Reflect", "Plan", "Workflow", "SDK", "提示詞", "生成溫度", "語意查找", "StoreRule", "BusinessRule", "latest_retrieved_content"):
        assert internal_term not in visible_text


def test_builder_choice_cards_explain_real_differences_and_avoid_duplicate_categories():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    page = response.data.decode("utf-8")
    assert "不預設外部 OpenAI 相容 client，適合先從最小流程開始。" in page
    assert "同時使用 key/value 對照表與意思比對。" in page
    assert "檢查答案是否回應問題，失敗處理由下方參數決定。" in page
    reflect_step_start = page.index('id="builder-step-reflect"')
    reflect_step_end = page.index("</article>", reflect_step_start)
    reflect_step_markup = page[reflect_step_start:reflect_step_end]
    assert reflect_step_markup.count("data-choice-card") == 3
    assert "檢查回覆，不通過時重新規劃" not in reflect_step_markup
    assert "檢查回覆，不通過時停止" not in reflect_step_markup
    assert "檢查回覆品質" in reflect_step_markup
    assert "檢查資料依據" in reflect_step_markup


def test_builder_name_step_uses_text_field_not_choice_cards():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    page = response.data.decode("utf-8")
    name_step_start = page.index('id="builder-step-name"')
    name_step_end = page.index("</article>", name_step_start)
    name_step_markup = page[name_step_start:name_step_end]
    assert "這個 Agent 要如何命名？" in name_step_markup
    assert "data-name-input" in name_step_markup
    assert "choice-grid" not in name_step_markup
    assert "data-choice-card" not in name_step_markup
    assert "進階設定" not in name_step_markup


def test_builder_action_structured_result_is_primary_choice():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    page = response.data.decode("utf-8")
    action_step_start = page.index('id="builder-step-action"')
    action_step_end = page.index("</article>", action_step_start)
    action_step_markup = page[action_step_start:action_step_end]
    assert "最後的回覆應該長什麼樣子？" in action_step_markup
    assert "輸出固定格式" in action_step_markup
    assert "規則處理進階配置" in action_step_markup


def test_builder_advanced_settings_cover_workflow_steps():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    page = response.data.decode("utf-8")
    runtime_controls = (
        "規則處理進階配置",
        "直接讀取進階配置",
        "需求整理進階配置",
        "支援資料判斷進階配置",
        "關鍵字對照進階配置",
        "檢查失敗進階配置",
        "試跑安全限制",
        'name="class_name"',
        'name="memory_key"',
        'name="direct_memory_key"',
        'name="on_failure"',
        'name="entry_module"',
    )
    for control in runtime_controls:
        assert control in page
    for dead_control in ('name="task_goal"', 'name="success_criteria"', 'name="input_description"', 'name="input_fields"', 'name="output_guidance"', 'name="route_rule"', 'name="direct_rule"', 'name="criteria"'):
        assert dead_control not in page


def test_builder_parameter_fields_are_contextual_to_module_steps():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    page = response.data.decode("utf-8")
    retrieve_step_start = page.index('id="builder-step-retrieve"')
    retrieve_step_end = page.index("</article>", retrieve_step_start)
    retrieve_step_markup = page[retrieve_step_start:retrieve_step_end]
    plan_step_start = page.index('id="builder-step-plan"')
    plan_step_end = page.index("</article>", plan_step_start)
    plan_step_markup = page[plan_step_start:plan_step_end]
    action_step_start = page.index('id="builder-step-action"')
    action_step_end = page.index("</article>", action_step_start)
    action_step_markup = page[action_step_start:action_step_end]
    assert 'data-param-form data-step-key="retrieve"' in retrieve_step_markup
    assert "關鍵字對照進階配置" in retrieve_step_markup
    assert "key 對 value" in retrieve_step_markup
    assert 'name="keyword_pairs"' in retrieve_step_markup
    assert 'name="retrieve_name"' not in retrieve_step_markup
    assert 'name="retrieve_description"' not in retrieve_step_markup
    assert 'name="top_k"' not in retrieve_step_markup
    assert "支援資料判斷進階配置" in plan_step_markup
    assert 'name="retrieve_name"' in plan_step_markup
    assert 'name="retrieve_description"' in plan_step_markup
    assert 'data-param-form data-step-key="action"' in action_step_markup
    assert 'name="direct_memory_key"' in action_step_markup
    assert 'name="rule_pairs"' in action_step_markup
    assert 'name="output_guidance"' not in page
    assert 'name="system_prompt"' not in page
    assert 'name="temperature"' not in page
    assert 'name="model"' not in page


def test_builder_template_step_only_selects_runtime_presets():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    page = response.data.decode("utf-8")
    template_step_start = page.index('id="builder-step-template"')
    template_step_end = page.index("</article>", template_step_start)
    template_step_markup = page[template_step_start:template_step_end]
    assert "整理成自然回覆" in template_step_markup
    assert "需求判斷" in template_step_markup
    assert "規則處理進階配置" in template_step_markup
    assert "需求整理進階配置" in template_step_markup
    assert "支援資料判斷進階配置" in template_step_markup
    assert "關鍵字對照進階配置" in template_step_markup
    assert 'data-param-form data-step-key="template"' not in template_step_markup
    assert 'data-param-form data-step-key="action"' in template_step_markup
    assert 'data-param-form data-step-key="perceive"' in template_step_markup
    assert 'data-param-form data-step-key="plan"' in template_step_markup
    assert 'data-param-form data-step-key="retrieve"' in template_step_markup
    for dead_field in ('name="task_goal"', 'name="success_criteria"', 'name="formal_audience"', 'name="question_mode"', 'name="reply_style"', 'name="output_guidance"'):
        assert dead_field not in template_step_markup


def test_builder_template_step_parameters_update_runtime_source():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post("/playground/builder/state", json={"step": "template", "choice": "Custom Action"})
        client.post(
            "/playground/builder/state",
            json={
                "step": "action",
                "value": {
                    "rule_title": "處理規則",
                    "rule_pairs": "已查到資料 = 先摘要重點，再列出依據。",
                    "fallback": "沒有可處理內容。",
                    "prefix": "處理結果：",
                },
            },
        )
        custom_source = client.get("/playground/source/preview").data.decode("utf-8")
        client.post("/playground/builder/state", json={"step": "template", "choice": "Advisor"})
        client.post(
            "/playground/builder/state",
            json={
                "step": "perceive",
                "value": {
                    "welcome_message": "請描述目標與限制。",
                    "intent_pairs": "目標 = 想完成的結果\n限制 = 不可超過的條件",
                    "importance": "2.5",
                },
            },
        )
        client.post(
            "/playground/builder/state",
            json={"step": "plan", "value": {"retrieve_name": "內部支援資料", "retrieve_description": "用來判斷下一步。"}},
        )
        advisor_source = client.get("/playground/source/preview").data.decode("utf-8")

    assert "BUSINESS_RULE_TITLE = \"處理規則\"" in custom_source
    assert "已查到資料：先摘要重點，再列出依據。" in custom_source
    assert "CUSTOM_ACTION_FALLBACK = \"沒有可處理內容。\"" in custom_source
    assert "CUSTOM_ACTION_PREFIX = \"處理結果：\"" in custom_source
    assert "TextPerceive(client=openai_client" in advisor_source
    assert "welcome_message=\"請描述目標與限制。\"" in advisor_source
    assert "importance=2.5" in advisor_source
    assert "NextStepPlan(client=openai_client, retrieve_name=\"內部支援資料\", retrieve_description=\"用來判斷下一步。\")" in advisor_source


def test_new_builder_does_not_prefill_form_state():
    app = create_app()

    with app.test_client() as client:
        client.post("/playground/start/anonymous")
        response = client.get("/playground/builder")

    assert response.status_code == 200
    state = _builder_form_state(response.data.decode("utf-8"))
    assert state == {"choices": {}, "values": {}}


def test_runner_edit_settings_rehydrates_configured_builder_state():
    app = create_app()

    with app.test_client() as client:
        client.post("/playground/start/anonymous")
        client.post("/playground/builder/state", json={"step": "name", "value": "TSiP Rule Agent"})
        client.post("/playground/builder/state", json={"step": "template", "choice": "Custom Action"})
        client.post("/playground/builder/state", json={"step": "retrieve", "value": {"keyword_pairs": "tsip = TSiP support content"}})
        client.post(
            "/playground/builder/state",
            json={
                "step": "action",
                "value": {
                    "rule_title": "Escalation Rule",
                    "rule_pairs": "found = summarize with evidence",
                    "fallback": "No support content.",
                    "prefix": "Result: ",
                },
            },
        )
        runner_response = client.get("/playground/run")
        builder_response = client.get("/playground/builder")

    assert runner_response.status_code == 200
    assert "編輯設定".encode("utf-8") in runner_response.data
    assert builder_response.status_code == 200
    state = _builder_form_state(builder_response.data.decode("utf-8"))
    assert state["choices"]["template"] == "Custom Action"
    assert state["choices"]["action"] == "Custom"
    assert state["values"]["name"]["agent_name"] == "TSiP Rule Agent"
    assert state["values"]["retrieve"]["keyword_pairs"] == "tsip = TSiP support content"
    assert state["values"]["action"]["rule_title"] == "Escalation Rule"
    assert state["values"]["action"]["rule_pairs"] == "found = summarize with evidence"
    assert state["values"]["action"]["fallback"] == "No support content."
    assert state["values"]["action"]["prefix"] == "Result: "


def test_builder_key_value_parameters_use_repeatable_pair_editors():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    page = response.data.decode("utf-8")
    expected_outputs = (
        ('id="perceive-intent-pairs" name="intent_pairs" hidden data-pair-output', "新增意圖"),
        ('id="retrieve-keyword-pairs" name="keyword_pairs" hidden data-pair-output', "新增關鍵字內容"),
        ('id="action-custom-rule-pairs" name="rule_pairs" hidden data-pair-output', "新增處理規則"),
    )
    for output_markup, add_label in expected_outputs:
        assert output_markup in page
        assert add_label in page
    assert page.count('data-pair-editor') >= len(expected_outputs)
    assert page.count('data-pair-key') >= len(expected_outputs)
    assert page.count('data-pair-value') >= len(expected_outputs)
    assert page.count('data-pair-add') >= len(expected_outputs)
    assert 'id="retrieve-keyword-pairs" name="keyword_pairs" rows=' not in page
    assert 'id="retrieve-keyword-pairs" name="keyword_pairs" hidden data-pair-output> = </textarea>' not in page


def test_builder_finite_parameters_use_constrained_controls():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/builder")

    assert response.status_code == 200
    page = response.data.decode("utf-8")
    assert '<select id="action-direct-memory-key" name="direct_memory_key">' in page
    assert '<input id="action-direct-memory-key"' not in page
    expected_range_controls = (
        'id="perceive-importance" name="importance" type="range" min="0" max="5" step="0.1"',
    )
    for control in expected_range_controls:
        assert control in page
    assert page.count("data-range-output") >= len(expected_range_controls)
    assert 'name="top_k"' not in page
    assert 'name="similarity_weight"' not in page
    assert 'name="recency_weight"' not in page
    assert 'name="importance_weight"' not in page


def test_builder_rejects_unsupported_finite_parameter_values():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post("/playground/builder/state", json={"step": "action", "choice": "Reply"})
        client.post("/playground/builder/state", json={"step": "action", "value": {"direct_memory_key": "unsupported_key"}})
        client.post("/playground/builder/state", json={"step": "readiness", "value": {"entry_module": "unsupported_step"}})
        preview = client.get("/playground/source/preview").data.decode("utf-8")

    assert "unsupported_key" not in preview
    assert "unsupported_step" not in preview
    assert 'DirectAnswerAction(memory_key="latest_retrieved_content"' in preview
    assert 'entry_module="perceive"' in preview


def test_builder_expanded_module_choices_update_canonical_source():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post("/playground/builder/state", json={"step": "input", "choice": "Form"})
        form_source = client.get("/playground/source/preview").data.decode("utf-8")
        client.post("/playground/builder/state", json={"step": "perceive", "choice": "TextImage"})
        image_source = client.get("/playground/source/preview").data.decode("utf-8")
        client.post("/playground/builder/state", json={"step": "retrieve", "choice": "Semantic"})
        client.post(
            "/playground/builder/state",
            json={"step": "retrieve", "value": {"top_k": "7", "similarity_weight": "0.6", "recency_weight": "0.2", "importance_weight": "0.2"}},
        )
        semantic_source = client.get("/playground/source/preview").data.decode("utf-8")
        client.post("/playground/builder/state", json={"step": "action", "choice": "Structured"})
        structured_source = client.get("/playground/source/preview").data.decode("utf-8")
        client.post("/playground/builder/state", json={"step": "reflect", "choice": "EvidenceStop"})
        reflect_source = client.get("/playground/source/preview").data.decode("utf-8")
        client.post(
            "/playground/builder/state",
            json={"step": "readiness", "value": {"entry_module": "plan", "max_node_hops": "12", "max_revisit": "3", "timeout_sec": "45"}},
        )
        readiness_source = client.get("/playground/source/preview").data.decode("utf-8")

    assert "StructuredPerceive(" in form_source
    assert "TextImagePerceive(" in image_source
    assert "SemanticRetrieve(" in semantic_source
    assert "top_k=" not in semantic_source
    assert "similarity_weight=" not in semantic_source
    assert "RETRIEVE_CONFIG" not in semantic_source
    assert "StructuredAction(client=openai_client" in structured_source
    assert "EvidenceCheckReflect(on_failure=\"end\")" in reflect_source
    assert "entry_module=\"plan\"" in readiness_source
    assert "Gates(max_node_hops=12, max_revisit=3, timeout_sec=45.0)" in readiness_source


def test_runner_route_renders_task_shell():
    app = create_app()

    with app.test_client() as client:
        client.post("/playground/start/anonymous")
        response = client.get("/playground/run")

    assert response.status_code == 200
    assert "客戶回覆 Agent".encode("utf-8") in response.data
    assert b"data-result-thread hidden" in response.data
    assert b"data-result-surface hidden" in response.data
    assert b"aria-controls=\"trust-basis-panel\"" in response.data
    assert b"data-archetype" in response.data
    assert b"data-layout-variant=\"chat_first\"" in response.data
    assert b"data-attachment-input" in response.data
    assert b"data-adoption-state" in response.data
    assert b"data-next-step-strip" in response.data
    assert b"data-next-step-action=\"copy\"" in response.data
    assert b"data-save-panel" in response.data
    assert b"data-save-action" in response.data
    assert b"data-save-status" in response.data
    assert b"data-history-list" not in response.data
    assert "最近試跑".encode("utf-8") not in response.data
    assert "尚未儲存".encode("utf-8") in response.data
    assert "儲存狀態".encode("utf-8") not in response.data
    assert "模式".encode("utf-8") not in response.data
    assert "主要輸入".encode("utf-8") not in response.data
    assert "主要結果".encode("utf-8") not in response.data


def test_runner_trust_panel_partial_is_rendered():
    app = create_app()

    with app.test_client() as client:
        client.post("/playground/start/anonymous")
        response = client.get("/playground/run")

    assert response.status_code == 200
    assert b"id=\"trust-basis-panel\"" in response.data
    assert b"data-evidence-list" in response.data
    assert Path("playground_v2/templates/partials/trust_panel.html").exists()


def test_runner_without_source_redirects_to_builder():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/run")

    assert response.status_code == 302
    assert "/playground/builder" in response.headers["Location"]


def test_aihub_readonly_hides_creator_controls():
    app = create_app()

    with app.test_client() as client:
        response = client.get("/playground/run?mode=aihub_readonly")

    assert response.status_code == 200
    assert "已儲存".encode("utf-8") in response.data
    assert b"data-save-action" not in response.data
    assert "編輯設定".encode("utf-8") not in response.data
    assert "Python 原始碼".encode("utf-8") not in response.data
    assert "匯出 .py".encode("utf-8") not in response.data
    assert "尚未儲存到 AI Hub".encode("utf-8") not in response.data
    assert "模式".encode("utf-8") not in response.data
    assert "主要輸入".encode("utf-8") not in response.data
    assert "主要結果".encode("utf-8") not in response.data


def test_aihub_deep_link_skips_entry_and_downgrades_untrusted_editable():
    app = create_app()

    with app.test_client() as client:
        entry_response = client.get("/playground/?mode=aihub_readonly&agent_id=agent-1")
        runner_response = client.get("/playground/run?mode=aihub_editable")

    assert entry_response.status_code == 302
    assert "/playground/run" in entry_response.headers["Location"]
    assert runner_response.status_code == 200
    assert "已儲存".encode("utf-8") in runner_response.data
    assert "編輯設定".encode("utf-8") not in runner_response.data
    assert b"data-save-action" not in runner_response.data


def test_aihub_config_load_downgrades_editable_without_account_context():
    app = create_app()

    with app.test_client() as client:
        load_response = client.post(
            "/playground/aihub/config/load",
            json={"agent_id": "agent-2", "editable": True},
        )
        runner_response = client.get("/playground/run")

    assert load_response.status_code == 200
    assert load_response.json["mode"] == "aihub_readonly"
    assert load_response.json["integration_status"] == "stub"
    assert load_response.json["requires_real_aihub_api"] is True
    assert "已儲存".encode("utf-8") in runner_response.data
    assert b"data-save-action" not in runner_response.data


def test_aihub_editable_mode_shows_update_context():
    app = create_app()

    with app.test_client() as client:
        client.post("/playground/auth/login", data={"username": "creator", "password": "secret"})
        load_response = client.post(
            "/playground/aihub/config/load",
            json={"agent_id": "agent-3", "editable": True},
        )
        runner_response = client.get("/playground/run")

    assert load_response.status_code == 200
    assert load_response.json["mode"] == "aihub_editable"
    assert "已儲存".encode("utf-8") in runner_response.data
    assert b"data-save-action" in runner_response.data
    assert "編輯設定".encode("utf-8") in runner_response.data


def test_aihub_config_rejects_disallowed_origin():
    app = create_app()

    with app.test_client() as client:
        load_response = client.post(
            "/playground/aihub/config/load",
            json={"agent_id": "agent-2"},
            headers={"Origin": "https://evil.example"},
        )
        save_response = client.post(
            "/playground/aihub/config/save",
            headers={"Origin": "https://evil.example"},
        )

    assert load_response.status_code == 403
    assert load_response.json["loaded"] is False
    assert save_response.status_code == 403
    assert save_response.json["saved"] is False


def test_aihub_save_allows_creator_modes_only():
    app = create_app()

    with app.test_client() as client:
        client.post("/playground/start/anonymous")
        anonymous_response = client.post("/playground/aihub/config/save")
        client.post("/playground/auth/login", data={"username": "creator", "password": "secret"})
        signed_in_response = client.post("/playground/aihub/config/save")
        runner_response = client.get("/playground/run")

    assert anonymous_response.status_code == 200
    assert anonymous_response.json["saved"] is True
    assert signed_in_response.status_code == 200
    assert signed_in_response.json["saved"] is True
    assert signed_in_response.json["integration_status"] == "stub"
    assert signed_in_response.json["requires_real_aihub_api"] is True
    assert "已儲存".encode("utf-8") in runner_response.data


def test_static_assets_are_served():
    app = create_app()

    with app.test_client() as client:
        css_response = client.get("/static/css/app.css")
        js_response = client.get("/static/js/runner/runner-page.js")
        module_response = client.get("/static/js/runner/scene-profile.js")
        dialog_response = client.get("/static/js/shared/dialog.js")

    assert css_response.status_code == 200
    assert b"--fluent-blue" in css_response.data
    assert b"--amd-red" in css_response.data
    assert js_response.status_code == 200
    assert b"data-trust-toggle" in js_response.data
    assert b"/playground/run/execute" in js_response.data
    assert b"/playground/source/export" not in js_response.data
    assert b"data-code-toggle" not in js_response.data
    assert b"setAdoptionState" in js_response.data
    assert b"bindNextStepStrip" in js_response.data
    assert b"bindAttachmentPicker" in js_response.data
    assert b"showSavePanel" in js_response.data
    assert b"setTrustEvidence" in js_response.data
    assert b"prependHistoryItem" not in js_response.data
    entry_js_response = client.get("/static/js/entry/entry-page.js")
    assert entry_js_response.status_code == 200
    assert "正在登入".encode("utf-8") in entry_js_response.data
    assert module_response.status_code == 200
    assert b"/playground/run/profile" in module_response.data
    assert dialog_response.status_code == 200
    assert b"dataset.open" in dialog_response.data


def test_route_map_source_and_runner_endpoints():
    app = create_app()

    with app.test_client() as client:
        client.post("/playground/start/anonymous")
        profile_response = client.get("/playground/run/profile")
        execute_response = client.post("/playground/run/execute", json={"message": "hello"})
        preview_response = client.get("/playground/source/preview")
        export_response = client.post("/playground/source/export")

    assert profile_response.status_code == 200
    assert profile_response.json["primary_input_kind"] == "chat"
    assert profile_response.json["layout_fit"]["chat_first"] == "preferred"
    assert "form_first" in profile_response.json["layout_fit"]
    assert "result_first" in profile_response.json["layout_fit"]
    assert execute_response.status_code == 200
    assert execute_response.json["status"] == "completed"
    assert "workflow_id" not in execute_response.json
    assert "visit_counts" not in execute_response.json
    assert "abort_reason" not in execute_response.json
    assert "source_execution" not in execute_response.json
    assert "來源：目前輸入內容。" in execute_response.json["result"]["evidence"]
    assert "source subset supported: True" not in execute_response.json["result"]["evidence"]
    assert "source workflow: Customer Helper" not in execute_response.json["result"]["evidence"]
    assert "history_item" not in execute_response.json
    assert preview_response.status_code == 200
    assert b"from agentic_sdk import Workflow" in preview_response.data
    assert b"workflow_name=\"Customer Helper\"" in preview_response.data
    assert export_response.status_code == 200
    assert "agentic_workflow.py" in export_response.headers["Content-Disposition"]


def test_runner_does_not_demo_memory_history_without_memory_engine():
    app = create_app()

    with app.test_client() as client:
        client.post("/playground/start/anonymous")
        first = client.post("/playground/run/execute", json={"message": "first"})
        second = client.post("/playground/run/execute", json={"message": "second"})
        response = client.get("/playground/run")

    assert first.status_code == 200
    assert second.status_code == 200
    assert "最近試跑".encode("utf-8") not in response.data
    assert b"data-history-list" not in response.data
    assert b"completed" not in response.data


def test_runner_execute_returns_readable_attachment_error():
    app = create_app()

    with app.test_client() as client:
        client.post("/playground/start/anonymous")
        response = client.post("/playground/run/execute", json={"attachments": [{"name": "missing-mime"}]})

    assert response.status_code == 200
    assert response.json["status"] == "input_error"
    assert response.json["error"] == "附件內容不完整。"
    assert "detail" not in response.json
    assert "raw" not in response.json["final_message"].lower()
    assert "source_execution" not in response.json


def test_runner_execute_uses_supported_source_metadata():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post("/playground/builder/state", json={"step": "template", "choice": "Recommendation"})
        response = client.post("/playground/run/execute", json={"message": "recommend"})

    assert response.status_code == 200
    assert "source_execution" not in response.json
    assert response.json["scene_profile"]["layout_variant"] == "result_first"


def test_builder_state_updates_canonical_source_and_runner_profile():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        update_response = client.post(
            "/playground/builder/state",
            json={"step": "template", "choice": "Recommendation"},
        )
        preview_response = client.get("/playground/source/preview")
        profile_response = client.get("/playground/run/profile")

    assert update_response.status_code == 200
    assert update_response.json["workflow_summary"]["name"] == "建議型 Agent"
    assert b"Recommendation Helper" in preview_response.data
    assert profile_response.json["layout_variant"] == "result_first"
    assert profile_response.json["layout_fit"]["result_first"] == "preferred"
    assert update_response.json["workflow_summary"]["template"] == "建議卡"


def test_builder_name_field_updates_canonical_source_and_summary():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        update_response = client.post(
            "/playground/builder/state",
            json={"step": "name", "value": "TSiP 顧問 Agent"},
        )
        preview_response = client.get("/playground/source/preview")
        runner_response = client.get("/playground/run")

    assert update_response.status_code == 200
    assert update_response.json["workflow_summary"]["name"] == "TSiP 顧問 Agent"
    assert 'workflow_name="TSiP 顧問 Agent"'.encode("utf-8") in preview_response.data
    assert "TSiP 顧問 Agent".encode("utf-8") in runner_response.data


def test_builder_module_parameters_update_canonical_source():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        retrieve_response = client.post(
            "/playground/builder/state",
            json={"step": "retrieve", "value": {"keyword_pairs": "tsip, 供應鏈 = TSiP 供應鏈說明。\n售後服務 = 保固窗口說明。"}},
        )
        client.post("/playground/builder/state", json={"step": "action", "choice": "Generative"})
        action_response = client.post(
            "/playground/builder/state",
            json={"step": "action", "value": {"system_prompt": "請使用台灣繁體中文，先給結論。"}},
        )
        plan_response = client.post("/playground/builder/state", json={"step": "plan", "choice": "RouteBySupport"})
        reflect_response = client.post("/playground/builder/state", json={"step": "reflect", "choice": "RetryPlan"})
        preview_response = client.get("/playground/source/preview")

    assert retrieve_response.status_code == 200
    assert action_response.status_code == 200
    assert plan_response.status_code == 200
    assert reflect_response.status_code == 200
    preview = preview_response.data.decode("utf-8")
    assert '"keywords": [' in preview
    assert '"tsip"' in preview
    assert '"供應鏈"' in preview
    assert '"content": "TSiP 供應鏈說明。"' in preview
    assert '"售後服務"' in preview
    assert '"content": "保固窗口說明。"' in preview
    assert 'GenerativeAction(client=openai_client)' in preview
    assert "ACTION_CONFIG" not in preview
    assert "請使用台灣繁體中文" not in preview
    assert "system_prompt=" not in preview
    assert 'NextStepPlan(client=openai_client, retrieve_name="支援資料"' in preview
    assert 'ResponseCheckReflect(client=openai_client, on_failure="retry_plan")' in preview


def test_builder_advanced_parameters_update_canonical_source():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post("/playground/builder/state", json={"step": "template", "choice": "Custom Action"})
        custom_response = client.post(
            "/playground/builder/state",
            json={
                "step": "action",
                "value": {
                    "class_name": "SupportDecisionAction",
                    "memory_key": "latest_support_note",
                    "rule_title": "支援資料不足時",
                    "rule_pairs": "支援資料不足 = 先說明缺少哪些資訊，再請使用者補充。",
                    "fallback": "沒有可用支援資料。",
                    "prefix": "處理結果：",
                },
            },
        )
        client.post("/playground/builder/state", json={"step": "plan", "choice": "RouteBySupport"})
        plan_response = client.post(
            "/playground/builder/state",
            json={"step": "plan", "value": {"system_prompt": "只有需要佐證時才查詢支援資料。"}},
        )
        client.post("/playground/builder/state", json={"step": "reflect", "choice": "ResponseRetry"})
        reflect_response = client.post("/playground/builder/state", json={"step": "reflect", "value": {"on_failure": "end"}})
        preview_response = client.get("/playground/source/preview")

    assert custom_response.status_code == 200
    assert plan_response.status_code == 200
    assert reflect_response.status_code == 200
    preview = preview_response.data.decode("utf-8")
    assert "class SupportDecisionAction" in preview
    assert 'CUSTOM_ACTION_MEMORY_KEY = "latest_support_note"' in preview
    assert 'BUSINESS_RULE_TITLE = "支援資料不足時"' in preview
    assert 'BUSINESS_RULE_INSTRUCTION = "支援資料不足：先說明缺少哪些資訊，再請使用者補充。"' in preview
    assert 'CUSTOM_ACTION_FALLBACK = "沒有可用支援資料。"' in preview
    assert 'CUSTOM_ACTION_PREFIX = "處理結果："' in preview
    assert "action=SupportDecisionAction()" in preview
    assert "PLAN_CONFIG" not in preview
    assert "只有需要佐證時才查詢支援資料" not in preview
    assert "system_prompt=" not in preview
    assert 'ResponseCheckReflect(client=openai_client, on_failure="end")' in preview


def test_builder_concrete_parameters_are_emitted_for_advanced_paths():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post(
            "/playground/builder/state",
            json={"step": "template", "value": {"task_goal": "協助判斷方案。", "success_criteria": "必須列出結論與依據。"}},
        )
        client.post("/playground/builder/state", json={"step": "input", "choice": "Form"})
        client.post(
            "/playground/builder/state",
            json={"step": "input", "value": {"input_description": "固定欄位收件。", "input_fields": "product = 產品名稱\nissue = 問題描述", "welcome_message": "請提供需求。", "importance": "2.5"}},
        )
        client.post("/playground/builder/state", json={"step": "perceive", "choice": "Structured"})
        client.post(
            "/playground/builder/state",
            json={"step": "perceive", "value": {"input_label": "客戶需求", "welcome_message": "請提供需求。", "intent_pairs": "售後服務 = 保固退換貨\n產品諮詢 = 方案比較", "importance": "2.5", "model": "perceive-local"}},
        )
        client.post("/playground/builder/state", json={"step": "retrieve", "choice": "Hybrid"})
        client.post(
            "/playground/builder/state",
            json={"step": "retrieve", "value": {"fallback": "沒有支援資料。", "keyword_pairs": "保固 = 保固說明。", "top_k": "5", "similarity_weight": "0.7", "recency_weight": "0.2", "importance_weight": "0.1"}},
        )
        client.post("/playground/builder/state", json={"step": "plan", "choice": "RouteBySupport"})
        client.post("/playground/builder/state", json={"step": "plan", "value": {"retrieve_name": "產品資料", "retrieve_description": "保固與售後資料。", "system_prompt": "資料不足時才查。", "model": "plan-local"}})
        client.post("/playground/builder/state", json={"step": "action", "choice": "Structured"})
        client.post("/playground/builder/state", json={"step": "action", "value": {"system_prompt": "請輸出 JSON。", "model": "answer-local", "temperature": "0.2"}})
        client.post("/playground/builder/state", json={"step": "reflect", "choice": "ResponseRetry"})
        client.post("/playground/builder/state", json={"step": "reflect", "value": {"criteria": "不可捏造資料。", "model": "reflect-local", "on_failure": "retry_plan"}})
        preview = client.get("/playground/source/preview").data.decode("utf-8")

    assert '"label": "售後服務"' in preview
    assert '"intent": "保固退換貨"' in preview
    assert 'StructuredPerceive(client=openai_client, welcome_message="請提供需求。", options=' in preview
    assert 'importance=2.5' in preview
    assert "HybridRetrieve(" in preview
    assert '"keywords": [\n                          "保固"' in preview
    assert 'fallback="沒有支援資料。"' in preview
    assert 'NextStepPlan(client=openai_client, retrieve_name="產品資料", retrieve_description="保固與售後資料。")' in preview
    assert 'StructuredAction(client=openai_client)' in preview
    assert 'ResponseCheckReflect(client=openai_client, on_failure="retry_plan")' in preview
    for removed_config in ("TASK_CONFIG", "INPUT_CONFIG", "PERCEIVE_CONFIG", "RETRIEVE_CONFIG", "ACTION_CONFIG", "PLAN_CONFIG", "REFLECT_CONFIG"):
        assert removed_config not in preview
    for ignored_value in ("協助判斷方案", "固定欄位收件", "資料不足時才查", "請輸出 JSON", "不可捏造資料", "top_k", "similarity_weight"):
        assert ignored_value not in preview
    assert "perceive-local" not in preview
    assert "plan-local" not in preview
    assert "answer-local" not in preview
    assert "reflect-local" not in preview
    assert "model=" not in preview
    assert "temperature=" not in preview


def test_builder_concrete_parameters_are_emitted_for_direct_paths():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post("/playground/builder/state", json={"step": "retrieve", "choice": "None yet"})
        client.post("/playground/builder/state", json={"step": "retrieve", "value": {"fallback": "目前沒有支援資料。"}})
        client.post("/playground/builder/state", json={"step": "plan", "choice": "Direct"})
        client.post("/playground/builder/state", json={"step": "plan", "value": {"direct_rule": "使用者資訊足夠時直接回答。"}})
        client.post("/playground/builder/state", json={"step": "action", "choice": "Reply"})
        client.post("/playground/builder/state", json={"step": "action", "value": {"direct_memory_key": "latest_retrieved_content", "direct_fallback": "沒有可用結果。", "direct_prefix": "結果："}})
        client.post("/playground/builder/state", json={"step": "reflect", "choice": "Later"})
        client.post("/playground/builder/state", json={"step": "reflect", "value": {"criteria": "低風險 demo 可略過。"}})
        preview = client.get("/playground/source/preview").data.decode("utf-8")

    assert 'KeywordRetrieve(\n        items=[],\n        fallback="目前沒有支援資料。",\n    )' in preview
    assert 'DirectAnswerAction(memory_key="latest_retrieved_content", fallback="沒有可用結果。", prefix="結果：")' in preview
    assert "RETRIEVE_CONFIG" not in preview
    assert "direct_rule" not in preview
    assert "低風險 demo" not in preview


def test_builder_readme_starter_templates_update_source():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        openai_response = client.post("/playground/builder/state", json={"step": "template", "choice": "OpenAI client"})
        openai_preview = client.get("/playground/source/preview")
        custom_response = client.post("/playground/builder/state", json={"step": "template", "choice": "Custom Action"})
        custom_preview = client.get("/playground/source/preview")

    assert openai_response.status_code == 200
    assert openai_response.json["workflow_summary"]["name"] == "自然回覆 Agent"
    assert b"from openai import OpenAI" in openai_preview.data
    openai_source = openai_preview.data.decode("utf-8")
    assert "GenerativeAction(client=openai_client" in openai_source
    assert "ACTION_CONFIG" not in openai_source
    assert "可直接發送的正式回覆" not in openai_source
    assert "model=" not in openai_source
    assert "temperature=" not in openai_source
    assert custom_response.json["workflow_summary"]["name"] == "規則處理 Agent"
    assert b"class BusinessRule" in custom_preview.data
    assert b"CUSTOM_ACTION_MEMORY_KEY" in custom_preview.data
    assert b"action=BusinessRule()" in custom_preview.data


def test_builder_workflow_templates_map_to_sdk_module_presets():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post("/playground/builder/state", json={"step": "template", "choice": "Advisor"})
        advisor_source = client.get("/playground/source/preview").data.decode("utf-8")
        client.post("/playground/builder/state", json={"step": "template", "choice": "Recommendation"})
        recommendation_source = client.get("/playground/source/preview").data.decode("utf-8")
        client.post("/playground/builder/state", json={"step": "template", "choice": "Review"})
        review_source = client.get("/playground/source/preview").data.decode("utf-8")

    assert "workflow_name=\"Advisor Helper\"" in advisor_source
    assert "TextPerceive(client=openai_client" in advisor_source
    assert "welcome_message=\"請先描述你想完成的事" in advisor_source
    assert '"label": "目標"' in advisor_source
    assert "NextStepPlan(client=openai_client" in advisor_source
    assert "KeywordRetrieve(" in advisor_source
    assert "GenerativeAction(client=openai_client" in advisor_source
    assert "請用一步步詢問" not in advisor_source
    assert "ACTION_CONFIG" not in advisor_source
    assert "PLAN_CONFIG" not in advisor_source
    assert "model=" not in advisor_source
    assert "temperature=" not in advisor_source
    assert "workflow_name=\"Recommendation Helper\"" in recommendation_source
    assert "TextPerceive(client=openai_client)" in recommendation_source
    assert "NextStepPlan(client=openai_client" in recommendation_source
    assert "HybridRetrieve(" in recommendation_source
    assert "StructuredAction(client=openai_client)" in recommendation_source
    assert "請用一步步詢問" not in recommendation_source
    assert "model=" not in recommendation_source
    assert "workflow_name=\"Review Summary Helper\"" in review_source
    assert "TextPerceive(client=openai_client)" in review_source
    assert "SemanticRetrieve(" in review_source
    assert "GenerativeAction(client=openai_client)" in review_source
    assert "請用一步步詢問" not in review_source
    assert "model=" not in review_source


def test_builder_advanced_action_choice_updates_source():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        update_response = client.post("/playground/builder/state", json={"step": "action", "choice": "Structured"})
        preview_response = client.get("/playground/source/preview")
        profile_response = client.get("/playground/run/profile")

    assert update_response.status_code == 200
    assert update_response.json["workflow_summary"]["name"] == "固定格式 Agent"
    assert b"Structured Result Helper" in preview_response.data
    assert profile_response.json["layout_variant"] == "form_first"


def test_builder_action_choice_clears_stale_action_specific_state():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post("/playground/builder/state", json={"step": "action", "choice": "Structured"})
        client.post("/playground/builder/state", json={"step": "action", "value": {"system_prompt": "請輸出 JSON。", "temperature": "0.2"}})
        client.post("/playground/builder/state", json={"step": "action", "choice": "Reply"})
        reply_preview = client.get("/playground/source/preview").data.decode("utf-8")
        reply_profile = client.get("/playground/run/profile")
        client.post("/playground/builder/state", json={"step": "template", "choice": "Custom Action"})
        client.post("/playground/builder/state", json={"step": "action", "choice": "Generative"})
        generative_preview = client.get("/playground/source/preview").data.decode("utf-8")

    assert "StructuredAction" not in reply_preview
    assert "GenerativeAction" not in reply_preview
    assert "system_prompt" not in reply_preview
    assert "Playground V2 profile hint: Structured Result" not in reply_preview
    assert 'DirectAnswerAction(memory_key="latest_retrieved_content", fallback="沒有命中任何條目。", prefix="")' in reply_preview
    assert reply_profile.json["layout_variant"] == "chat_first"
    assert "class BusinessRule" not in generative_preview
    assert "GenerativeAction(client=openai_client)" in generative_preview


def test_builder_document_input_choice_updates_source():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        update_response = client.post("/playground/builder/state", json={"step": "input", "choice": "Document"})
        preview_response = client.get("/playground/source/preview")
        profile_response = client.get("/playground/run/profile")

    assert update_response.status_code == 200
    assert update_response.json["workflow_summary"]["name"] == "審閱摘要 Agent"
    assert b"Document Review Helper" in preview_response.data
    assert profile_response.json["layout_variant"] == "result_first"


def test_runner_uses_source_derived_layout_surfaces():
    app = create_app()

    with app.test_client() as client:
        client.get("/playground/builder")
        client.post("/playground/builder/state", json={"step": "template", "choice": "Recommendation"})
        recommendation_response = client.get("/playground/run")
        client.post("/playground/builder/state", json={"step": "input", "choice": "Form"})
        form_response = client.get("/playground/run")

    assert b"data-layout-variant=\"result_first\"" in recommendation_response.data
    assert "建議的下一步".encode("utf-8") in recommendation_response.data
    assert "簡短決策說明".encode("utf-8") not in recommendation_response.data
    assert "建議卡".encode("utf-8") not in recommendation_response.data
    assert b"data-layout-variant=\"form_first\"" in form_response.data
    assert "結構化需求內容".encode("utf-8") in form_response.data
    assert "結構化需求欄位".encode("utf-8") not in form_response.data
    assert "摘要卡".encode("utf-8") not in form_response.data