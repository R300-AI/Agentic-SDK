...
只有一個來源時可以不寫成串列。取回的 repo 與解開的壓縮檔都留在快取裡（預設在使用者的 cache 目錄，或由 `AGENTIC_SDK_SKILL_PACKAGES` 指定）：同一個網址加同一個版本只會取回一次，同一份壓縮檔也只會解開一次，所以跑過一次的流程在沒有網路或 repo 被刪掉時仍然起得來。取回發生在建立規劃模組的時候，不是在執行的時候；取到一半失敗不會留下半個技能包。資料夾來源就地使用，不進快取。
