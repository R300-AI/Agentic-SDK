...
`PlanCheckReflect` 用模型確認規劃模組選的步驟能不能執行、檢索有沒有正常完成，例如規劃選了一個不存在的步驟。模型讀到的內容是：最近一次規劃決定的 `thought`、規劃模組這一次可選的步驟、最近一次檢索內容的前 500 字與 `hit_count`、感知結果的前 500 字，以及完整對話紀錄。它不讀行動結果。
