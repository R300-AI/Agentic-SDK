...
`max_prompt_tokens` 是另一種上限。它不在訪問模組之前檢查，因為要送出去多少只有在記憶把內容組好的那一刻才算得出來，所以它在那裡生效，而 `Gates` 其餘各項都在訪問之前檢查。

超出時按三個階段處理。

記得住自己的記憶先把最舊的合成成主題，那一段話因此留在索引裡而不是消失。還是裝不下就遮蔽：這個執行取回來的東西——工具回覆、取用過的技能文字——拿掉細節，留下一句說明它原本是什麼。規劃因此仍然看得到這件事做過，不會再查一次；使用者說的話與 Agent 給的答案不遮，遮了會變成一段宣稱有人說過話、又不肯說是什麼話的對話。理由見 [ADR-0018](../adr/0018-what-was-fetched-is-masked-not-cleared.md)。前幾輪帶過來的檢索證據也一樣遮：它是每一輪證據滾成的一塊，放在系統那一層，不遮的話它會活得比請求裡其他每一樣都久。這一輪自己查到的證據不遮，那是正在寫的答案要對照的東西。遮完還是裝不下，才從對話最舊的一端整則放掉；已經遮成一行的那幾則不放掉——把它們縮成一行就是為了留著。工具回覆跟著叫它的那則訊息一起放掉，單獨留著等於一個沒有問題的答案。

三個階段都沒有固定界線，都是裝不下才開始、裝得下就停，共用同一個數字。系統提示（含索引）、檢索證據與這一輪的內容不動——切進那三樣會改變這一輪在問什麼。

「這一輪」指最後一次模型發言之後的全部內容，以及使用者最後說的那一句，兩者取較早的那個位置。取較早的原因有兩個：規劃模組會在問題後面補一則使用者角色的技能文字，只看最後一則使用者訊息會讓問題本身變成可放掉的；而呼叫工具的那一輪結束在工具回覆上，最後一則訊息既不是問題也不是使用者發的。

**這個數字是對話被衡量的標準，不是請求大小的保證。** 對話全部放掉之後還是超出時，請求照樣送出去——端點退件比回答一個沒有人問過的問題清楚。它也是估的，因為 OpenAI 相容端點不保證提供計數介面；圖片不計入，那要看端點自己怎麼切塊。
...
| `timeout` | 花的時間超過上限 |
| `budget_exhausted` | 這一輪可用的內容量用完了（超出 `max_prompt_tokens` 不會中止一輪，只會讓對話變短，所以目前沒有人發出這個原因） |
