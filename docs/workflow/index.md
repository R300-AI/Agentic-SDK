...
| 同一步驟重複次數 | `max_revisit` | `5` | 同一個步驟最多重複執行的次數。規劃模組不計入。 |
...
規劃模組不計入 `max_revisit`，因為它在每次檢索與每次反思之後各被造訪一次，造訪次數等於 1 加檢索次數加反思次數，已經被另外兩項限制住。預設值下最長路徑是感知 1、檢索 5、反思 5、規劃 11、行動 1，共 23 步，低於 `max_node_hops` 的 50。一般應用程式可保留預設值；當流程有明確的外部等待或重試需求時，再依實際執行紀錄調整。
